...
async function fetchQuestions() {
    try {
        const response = await fetch('http://localhost:8080/api/questions/Java');
        ...
        displayQuestion();
        startTimer(); // Start the timer when questions are successfully fetched
    } catch (error) {
...
function displayQuestion() {
    ...
    currentQuestion.options.forEach(option => {
        ...
        button.onclick = () => checkAnswer(option); // Add event listener to select the answer
...
function nextQuestion() {
    if (currentQuestionIndex < questions.length - 1) {
        ...
        saveQuizResult(); // Quiz completed, save and display the final score
    }
}
